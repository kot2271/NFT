...
task("transferFromERC721", "Transfers a token from one address to another")
  ...
  .setAction(
    async (
      ...
      await erc721.transferFrom(addressFrom, addressTo, tokenId);
      ...
      console.log(
        `ERC721 Token ${txTokenId} transferred from ${txAddressFrom} to ${txAddressTo}`
      );
    }
  );
